import api from '@/plugins/tAxios/index'
import type {
  TopListRes,
  AppDetailsInfoRes,
  TopResItem,
...
type PromiseTopListResType = T_AXIOS.PlAxiosPromise<TopListRes>
type TResponsePromise = T_AXIOS.TResponsePromise

// 对原始数据进行过滤处理
const topDataFilter = async (res: PromiseTopListResType) => {
  const topDataList = await res.then((data: TResponsePromise) => {
